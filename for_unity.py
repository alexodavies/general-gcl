...
import wandb
from utils import better_to_nx,  vis_grid, setup_wandb
from utils import lowres_vis_from_pyg as vis_from_pyg
...
def vis_vals(loader, dataset_name, num = 10000):
    ...
    data_directory = "/Users/alexdavies/Unity/Scatter Visualisation/Assets/Resources/Graph_Images/"
    ...
    for i in range(num):
        ...
        if f"{dataset_name}-val-{i}.png" in existing_files:
            pass
        else:
            skip_all = False
            break
    ...
    for i, data in enumerate(tqdm(val_data)):
        filename = os.getcwd() + f'/{dataset_name}-val-{i}.png'
        if i >= num or f"val-{i}.png" in existing_files:
            pass
        else:
            vis_from_pyg(data, filename=filename)

    os.chdir("/Users/alexdavies/Unity/Scatter Visualisation/Assets/Resources")
...
def run(args):
    ...
    embedder = UMAP(n_components=3, n_neighbors=50, n_jobs=6)
    ...
    for i, emb in enumerate(separate_embeddings):
        ...
        img_root = "/Users/alexdavies/Unity/Scatter Visualisation/Assets/Resources/Graph_Images"# os.getcwd() + '/original_datasets/' + names[i] + '/vals/*.png'

        img_paths = glob.glob(img_root+f"/*{names[i]}*.png")
        print(img_paths)

        filenames = [int(filename.split('val-')[-1][:-4]) for filename in img_paths]
        print(filenames)
    ...
    for i in range(len(names)):
        ...
        these_img_paths = glob.glob(img_root+f"/*{names[i]}*.png")
        # print(img_paths)

        filenames = [int(filename.split('val-')[-1][:-4]) for filename in these_img_paths]
        sort_inds = np.argsort(filenames).tolist()
    ...
    embedding_dataframe.to_csv("/Users/alexdavies/Unity/Scatter Visualisation/Assets/Resources/lowres_embeddings.csv")
