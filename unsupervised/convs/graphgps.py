    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.conv is not None:  # Local MPNN.
            ...
            assert torch.isfinite(h).all(), "NaNs detected after Message Passing in GPSConv"

            h = F.dropout(h, p=self.dropout, training=self.training)
            assert torch.isfinite(h).all(), "NaNs detected after Dropout in GPSConv"

            h = h + x
            assert torch.isfinite(h).all(), "NaNs detected after residual connection in GPSConv (MPNN part)"
            ...
            if self.norm1 is not None:
                if self.norm_with_batch:
                    ...
                    h = self.norm1(h)
            assert torch.isfinite(h).all(), "NaNs detected after Norm1 in GPSConv"
            hs.append(h)

        # Global attention transformer-style model.
        h, mask = to_dense_batch(x, batch)
        assert torch.isfinite(h).all(), "NaNs detected in input to Attention in GPSConv"
        ...
        assert torch.isfinite(h).all(), f"NaNs detected after Attention in GPSConv, {torch.sum(torch.isfinite(h))}"

        h = h[mask]
        h = F.dropout(h, p=self.dropout, training=self.training)
        h = h + x  # Residual connection.

        assert torch.isfinite(h).all(), "NaNs detected after residual connection in GPSConv (Attention part)"
        ...
        assert torch.isfinite(out).all(), "NaNs detected after combining local and global outputs in GPSConv"

        out = out + self.mlp(out)

        assert torch.isfinite(out).all(), "NaNs detected after MLP in GPSConv"
        if self.norm3 is not None:
            if self.norm_with_batch:
